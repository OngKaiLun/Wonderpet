...
public class Print_receipt extends JFrame{
    ...
    public Print_receipt(double total,int quan,double gst,double memdis,double finaltotal,Payment payment,ArrayList<domain.Product> tmpproductArray,double payamount){
        
        setLayout(new BorderLayout());
        for(int z=0 ;z<tmpproductArray.size();z++){
            String tmp="";
            tmp =String.format("\t%-3d%-30s\t\t%-5.2f\n",z+1,tmpproductArray.get(z).getPro_Name(),tmpproductArray.get(z).getPro_Price());
            produtlist +=tmp;
        }
       ...
       double round = (Math.ceil(((payment.getP_Amount()-0.05)*20)))/20;
        L1.setText("\t\t           BOBO Bistro\t\t\n"
                ...
                + "\tCASH\t\t\t= RM"+String.format("%.2f",payamount)+"\t\t\n"
                + "\tCG\t\t\t= RM"+String.format("%.2f",(payamount-round))+"\t\t\n"
                + "\t**********************************************************\t\t\n"
